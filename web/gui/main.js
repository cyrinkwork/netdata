...
function alarmsUpdateModal() {
    var active = '<h3>出发警报</h3><table class="table">';
    var all = '<h3>所有运行中的警报</h3><div class="panel-group" id="alarms_all_accordion" role="tablist" aria-multiselectable="true">';
    var footer = '<hr/><a href="https://github.com/netdata/netdata/tree/master/web/api/badges#netdata-badges" target="_blank">netdata badges</a> 自动刷新。不同的颜色表示不同的警报状态： <span style="color: #e05d44"><b>&nbsp;红色&nbsp;</b></span> 表示重要警报, <span style="color:#fe7d37"><b>&nbsp;橙色&nbsp;</b></span> 表示警告, <span style="color: #4c1"><b>&nbsp;亮绿色&nbsp;</b></span> 表示正常, <span style="color: #9f9f9f"><b>&nbsp;亮灰色&nbsp;</b></span> 表示未定义 (例如无资料或无状态), <span style="color: #000"><b>&nbsp;黑色&nbsp;</b></span> 表示尚未初始化。您可以复制这里的网址并将它们嵌入到任一个网页。<br/>netdata 可以发送这些警报通知。请参考 <a href="https://github.com/netdata/netdata/blob/master/health/notifications/health_alarm_notify.conf" target="_blank">这个设定文件</a> 了解更多信息。';
    ...
    NETDATA.alarms.get('all', function (data) {
        ...
        document.getElementById('alarms_log').innerHTML = '<h3>警报日志</h3><table id="alarms_log_table"></table>';
...
function initializeDynamicDashboardWithData(data) {
    if (data !== null) {
        ...
        document.title = options.hostname + ' netdata仪表盘';
...
function getGithubLatestVersion(callback) {
    ...
    $.ajax({
        ...
        .done(function (data) {
            ...
            versionLog('从github获取的最新版本是 ' + data);
            callback(data);
        })
        .fail(function () {
            versionLog('从github系在最新版本 ID 失败');
            callback(null);
        });
}
...
function notifyForUpdate(force) {
    ...
    checkForUpdateByVersion(force, function (sha1, sha2) {
        ...
        if (sha1 === null) {
            save = false;
            versionLog('<p><big>读取netdata版本失败</big></p><p>You can always get the latest netdata from <a href="https://github.com/netdata/netdata" target="_blank">its github page</a>.</p>');
        } else if (sha2 === null) {
            save = false;
            versionLog('<p><big>从github获取最新版本netdata失败</big></p><p>You can always get the latest netdata from <a href="https://github.com/netdata/netdata" target="_blank">its github page</a>.</p>');
        } else if (versionsMatch(sha1, sha2)) {
            save = true;
            versionLog('<p><big>已是最新版本netdata</big></p><p>还未更新？<br/>或许我们还需要一些前进的动力</p><p>如果您还没有做好准备，请， <a href="https://github.com/netdata/netdata" target="_blank">到github给netdata <b><i class="fas fa-star"></i></b> at its github page</a>.</p>');
        } else {
            save = true;
